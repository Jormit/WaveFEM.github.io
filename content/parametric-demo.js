...
function buildHornGeometry(p) {
    const tw = p.throat_w, th = p.throat_h, tl = p.throat_l;
    const aw = p.aperture_w, ah = p.aperture_h, hl = p.horn_l;
    const t = p.thickness;
    const zt = tl + hl;

    const two = tw + 2 * t, tho = th + 2 * t;
    const awo = aw + 2 * t, aho = ah + 2 * t;

    const hw = tw / 2, hh = th / 2, haw = aw / 2, hah = ah / 2;
    const hwo = two / 2, hho = tho / 2, hawo = awo / 2, haho = aho / 2;

    const O = [
        [-hwo, -hho, 0], [hwo, -hho, 0], [hwo, hho, 0], [-hwo, hho, 0],
        [-hwo, -hho, tl], [hwo, -hho, tl], [hwo, hho, tl], [-hwo, hho, tl],
        [-hawo, -haho, zt], [hawo, -haho, zt], [hawo, haho, zt], [-hawo, haho, zt],
    ];
    const I = [
        [-hw, -hh, t], [hw, -hh, t], [hw, hh, t], [-hw, hh, t],
        [-hw, -hh, tl], [hw, -hh, tl], [hw, hh, tl], [-hw, hh, tl],
        [-haw, -hah, zt], [haw, -hah, zt], [haw, hah, zt], [-haw, hah, zt],
    ];
    ...
    function quadAuto(a, b, c, d, reverse) {
        const p0 = vertexPositions[a], p1 = vertexPositions[b];
        const p2 = vertexPositions[c], p3 = vertexPositions[d];
    ...
    for (let i = 0; i < ind.length; i++) {
        const vi = ind[i];
        positions.push(vertexPositions[vi][0], vertexPositions[vi][1], vertexPositions[vi][2]);
    }
